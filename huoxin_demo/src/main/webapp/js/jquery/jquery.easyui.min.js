...
if(opts.validType){
//var _2f4=/([a-zA-Z_]+)(.*)/.exec(opts.validType); // modify by zhangzha on 20110609
var spt = opts.validType.split(" ");
for(var i=0;i<spt.length;i++){
    var _2f4=/([a-zA-Z0-9_]+)(.*)/.exec(spt[i]);
	var rule=opts.rules[_2f4[1]];
	if(_2f1&&rule){
	var _2f5=eval(_2f4[2]);
	if(!rule["validator"](box[0],_2f5)){ // modify by zhangzha on 20110609 -- (_2f1,_2f5) to (box[0],_2f5)
	box.addClass("validatebox-invalid");
...
$.fn.validatebox.defaults={required:false,validType:null,missingMessage:"???????????????",invalidMessage:null,rules:{email:{validator:function(_2fb){
...
},message:"?????????E-mail???"},url:{validator:function(_2fc){
...
},message:"??????????????URL"},length:{validator:function(_2fd,_2fe){
